      ...
      <nav className="flex-1 overflow-y-auto py-3 px-2 space-y-0.5">
        ...
        <Link
          ...
          className={`flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-sm font-semibold transition-all
            ${pathname === '/biblioteca' ? 'bg-[#000FFF]/10 text-[#000FFF] font-bold' : 'text-gray-500 hover:bg-gray-50'}`}
        >
          <BibliotecaIcon />
        ...
        <Link
          ...
          onClick={onNavigate}
          className={`flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-sm font-semibold transition-all
            ${pathname === '/chat' ? 'bg-[#000FFF]/10 text-[#000FFF] font-bold' : 'text-gray-500 hover:bg-gray-50'}`}
        >
          <ChatIcon />
          <span>Bot João</span>
        ...
        <Link
          ...
          onClick={onNavigate}
          className={`flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-sm font-semibold transition-all
            ${pathname === '/notificacoes' ? 'bg-[#000FFF]/10 text-[#000FFF] font-bold' : 'text-gray-500 hover:bg-gray-50'}`}
        >
          <BellIcon />
      ...
      <div className="border-t border-gray-100 px-4 py-3 space-y-1">
        {profile.role !== 'consultant' && (
          <Link
            href="/admin"
            onClick={onNavigate}
            className={`flex items-center gap-2 text-xs py-1.5 font-semibold transition-colors ${pathname.startsWith('/admin') ? 'text-[#000FFF]' : 'text-gray-400 hover:text-[#000FFF]'}`}
          >
...
function TrailIcon({ active }: { active: boolean }) {
  return <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={active ? 'white' : 'currentColor'} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>
}
function ChatIcon() {
